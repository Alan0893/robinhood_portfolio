...
@app.route('/api/what-if/save', methods=['POST'])
def api_save_what_if():
    ...
    try:
        ...
        scenario_name = data.get('name', f'{symbol} - ${amount}')
        ...
        scenarios_file = 'what_if_scenarios.json'
        scenarios = []
        ...
        print(f"Error saving what-if scenario: {e}")
        return jsonify({'success': False, 'message': str(e)}), 500


@app.route('/api/what-if/list', methods=['GET'])
def api_list_what_if():
    """Get list of saved what-if scenarios."""
    # Check if user is logged in
    if not is_logged_in():
        return jsonify({'success': False, 'message': 'Please login first'}), 401
    try:
        ...
        print(f"Error loading what-if scenarios: {e}")
        return jsonify({'success': False, 'message': str(e)}), 500


@app.route('/api/what-if/delete/<int:scenario_id>', methods=['DELETE'])
def api_delete_what_if(scenario_id):
    ...
    try:
        ...
        print(f"Error deleting what-if scenario: {e}")
        return jsonify({'success': False, 'message': str(e)}), 500
